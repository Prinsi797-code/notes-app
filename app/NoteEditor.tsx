...
type ActivePanel = null | 'format' | 'color' | 'category' | 'image' | 'font' | 'theme' | 'more';
...
export default function NoteEditorModal() {
  ...
  const [selectedThemeId, setSelectedThemeId] = useState<string>('none');  // per-note theme
  ...
  useEffect(() => {
    if (noteId && notes.length > 0 && !isLoaded) {
      ...
      setSelectedThemeId('none');   // reset theme for new note
      setIsLoaded(false);
    }
  ...
  const handleSave = async () => {
    ...
    const formData: NoteFormData = {
      ...
      themeId: selectedThemeId,   // save per-note theme
    };
    try {
      ...
      <Animated.View style={[styles.panel, {
        ...
        {activePanel === 'format' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>{t('model.textFormatting')}</Text>
            <TextFormatter textStyle={textStyle} onToggle={handleToggleTextStyle} label="" />
          </View>
        )}
        {activePanel === 'color' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>{t('model.noteColor')}</Text>
            <ColorSelector selectedColor={selectedColor} onSelectColor={setSelectedColor} label="" />
          </View>
        )}
        {activePanel === 'category' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>{t('model.category')}</Text>
            <CategoryFilter selectedCategory={category} onSelectCategory={setCategory as any} />
          </View>
        )}
        {activePanel === 'image' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>{t('model.attachImage')}</Text>
            <ImageUploader images={images} onPickImage={pickImage} onRemoveImage={removeImage} loading={imageLoading} label="" />
          </View>
        )}
        {activePanel === 'font' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>FONT STYLE</Text>
        ...
        {/* Per-note theme panel */}
        {activePanel === 'theme' && (
          <View style={styles.panelInner}>
            <Text style={[styles.panelTitle, { color: colors.textSecondary }]}>NOTE THEME</Text>
            <NoteThemeSelector
              colors={colors}
  ...
  const body = (
    <>
      <View style={styles.header}>
        <TouchableOpacity onPress={handleCancel} style={styles.headerBtn} disabled={isAdLoading || saving}>
        ...
        >
          {saving ? <Text style={styles.doneTxt}>...</Text> : <Ionicons name="checkmark" size={20} color="#fff" />}
        </TouchableOpacity>
      </View>
            ...
            {images.map((uri, idx) => {
              ...
              return (
                <View key={`img-${idx}`} style={{ marginBottom: 16 }}>
                  ...
                  <Image
                    source={{ uri }}
                    style={[styles.imgPreview, { borderColor: colors.border }, isDrawing && { backgroundColor: '#000' }]}
                    resizeMode={isDrawing ? 'contain' : 'cover'}
                  />
                  <TouchableOpacity onPress={() => saveImageToGallery(uri)} style={[styles.imgSaveBtn, { backgroundColor: colors.cardBackground }]}>
  ...
  // ── Root ─────────────────────────────────────────────────────────────────
  return (
    // <SafeAreaView style={[styles.root, { backgroundColor: hasThemeBg ? 'transparent' : colors.background }]}>
    <View style={[
      styles.root,
      {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }} keyboardVerticalOffset={0}>
        ...
        <View style={[styles.toolbar, { backgroundColor: colors.cardBackground, borderTopColor: colors.border }]}>
          <Btn active={activePanel === 'format'} colors={colors} onPress={() => togglePanel('format')}>
            <Text style={[styles.btnLabel, { color: activePanel === 'format' ? colors.primary : colors.textSecondary }]}>Aa</Text>
          </Btn>
          <Btn active={activePanel === 'category'} colors={colors} onPress={() => togglePanel('category')}>
            <Ionicons name="list" size={22} color={activePanel === 'category' ? colors.primary : colors.textSecondary} />
          </Btn>
          <Btn active={showTable} colors={colors} onPress={() => setShowTable(v => !v)}>
            <Ionicons name="grid-outline" size={22} color={showTable ? colors.primary : colors.textSecondary} />
          </Btn>
          <Btn active={activePanel === 'color'} colors={colors} onPress={() => togglePanel('color')}>
            <Ionicons name="color-palette-outline" size={22} color={activePanel === 'color' ? colors.primary : colors.textSecondary} />
          </Btn>
          <Btn active={activePanel === 'image'} colors={colors} onPress={() => togglePanel('image')}>
            <Ionicons name="attach" size={22} color={activePanel === 'image' ? colors.primary : colors.textSecondary} />
          </Btn>
          <Btn active={activePanel === 'font'} colors={colors} onPress={() => togglePanel('font')}>
            <Text style={[styles.fontBtnLabel, { color: activePanel === 'font' ? colors.primary : colors.textSecondary }]}>F</Text>
          </Btn>
          {/* ✅ Theme button */}
          <Btn active={activePanel === 'theme'} colors={colors} onPress={() => togglePanel('theme')}>
            <Ionicons name="image-outline" size={22} color={activePanel === 'theme' ? colors.primary : colors.textSecondary} />
          </Btn>
          <Btn active={false} colors={colors} onPress={() => router.push({ pathname: '/DrawingScreen', params: noteId ? { noteId } : {} })}>
            <View style={{ position: 'relative', width: 26, height: 26, alignItems: 'center', justifyContent: 'center' }}>
              ...
              <View style={styles.premiumDot}><Ionicons name="diamond" size={8} color="#fff" /></View>
            </View>
          </Btn>
          <Btn active={false} colors={colors} onPress={() => { if (activePanel) closePanel(); }}>
...
function Btn({ children, active, onPress, colors }: { children: React.ReactNode; active: boolean; onPress: () => void; colors: any }) {
  return (
    <TouchableOpacity onPress={onPress} style={[styles.toolbarBtn, active && { backgroundColor: colors.primary + '18' }]} activeOpacity={0.7}>
      {children}
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  root: { flex: 1 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: 50, paddingBottom: 4 },
  ...
  toolbarBtn: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 10, borderRadius: 8, marginHorizontal: 2 },
